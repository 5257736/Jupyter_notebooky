#Dvadsiaty treti zapas
P <- matrix(c(38/94,1-26/83,1-38/94,26/83), nrow=2, ncol=2)
print(P)
...
#https://uclabruins.com/sports/mens-volleyball/stats/2022/brigham-young/boxscore/29616
# UCLA vs BYU 
# 20:25  
# 25:23
# 25:16
...
byu231 <- read.csv2('z23byu.csv', sep=";", header = FALSE)[1:46,1]
byu232 <- read.csv2('z23byu.csv', sep=";", header = FALSE)[1:49,2]
byu233 <- read.csv2('z23byu.csv', sep=";", header = FALSE)[1:42,3]
byu234 <- read.csv2('z23byu.csv', sep=";", header = FALSE)[1:44,4]
...
byu231_sum<- count_runs_services_sideout(byu231,1:9) 
byu231_sum$num_runs
byu231_sum$A_points
byu231_sum$B_points
byu231_sum$n_A
byu231_sum$n_B
byu231_sum$A_services
byu231_sum$B_services

byu232_sum <- count_runs_services_sideout(byu232,1:9) 
byu232_sum$num_runs
byu232_sum$A_points
byu232_sum$B_points
byu232_sum$n_A
byu232_sum$n_B
byu232_sum$A_services
byu232_sum$B_services

byu233_sum <- count_runs_services_sideout(byu233,1:9)
byu233_sum$num_runs
byu233_sum$A_points
byu233_sum$B_points
byu233_sum$n_A
byu233_sum$n_B
byu233_sum$A_services
byu233_sum$B_services

byu234_sum <- count_runs_services_sideout(byu234,1:9)
byu234_sum$num_runs
byu234_sum$A_points
byu234_sum$B_points
byu234_sum$n_A
byu234_sum$n_B
byu234_sum$A_services
byu234_sum$B_services


num_runs <- rbind(rbind(rbind(byu231_sum$num_runs,byu232_sum$num_runs),byu233_sum$num_runs),byu234_sum$num_runs)
colSums(num_runs)

#Pre prvy set
(byu231_sum$n_A)/(byu231_sum$n_A+(byu231_sum$B_points- byu231_sum$n_B))
(byu231_sum$n_B)/(byu231_sum$n_B+(byu231_sum$A_points- byu231_sum$n_A))

P1 <- matrix(c(0.1,1-0.28,1-0.1,0.28), nrow=2, ncol=2)
print(P1)

#Pre druhy set
(byu232_sum$n_A)/(byu232_sum$n_A+(byu232_sum$B_points- byu232_sum$n_B))
(byu232_sum$n_B)/(byu232_sum$n_B+(byu232_sum$A_points- byu232_sum$n_A))

P2 <- matrix(c(0.36,1-0.3043478,1-0.36,0.3043478), nrow=2, ncol=2)
print(P2)

#Pre treti set
(byu233_sum$n_A)/(byu233_sum$n_A+(byu233_sum$B_points- byu233_sum$n_B))
(byu233_sum$n_B)/(byu233_sum$n_B+(byu233_sum$A_points- byu233_sum$n_A))

P3 <- matrix(c(0.7083333,1-0.5294118,1-0.7083333,0.5294118), nrow=2, ncol=2)
print(P3)

#Pre stvrty set
(byu234_sum$n_A)/(byu234_sum$n_A+(byu234_sum$B_points- byu234_sum$n_B))
(byu234_sum$n_B)/(byu234_sum$n_B+(byu234_sum$A_points- byu234_sum$n_A))

P4 <- matrix(c(0.4,1-0.1666667,1-0.4,0.1666667), nrow=2, ncol=2)
print(P4)
...
#Simulacia prvy set
byu231_sim_1000 <- sim_volley_games_proc(P1, max_points=25, num_games=1000, A_points=20, B_points=25)
#Simulacia druhy set
byu232_sim_1000 <- sim_volley_games_proc(P2, max_points=25, num_games=1000, A_points=25, B_points=23)
#Simulacia treti set
byu233_sim_1000 <- sim_volley_games_proc(P3, max_points=25, num_games=1000, A_points=25, B_points=16)
#Simulacia stvrty set
byu234_sim_1000 <- sim_volley_games_proc(P4, max_points=25, num_games=1000, A_points=25, B_points=18)


num_runs_sim <- rbind(rbind(rbind(byu231_sim_1000,byu232_sim_1000),byu233_sim_1000),byu234_sim_1000)
