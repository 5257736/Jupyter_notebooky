#Osemnasty zapas
P <- matrix(c(47/73,1-3/31,1-47/73,3/31), nrow=2, ncol=2)
print(P)
...
#https://uclabruins.com/sports/mens-volleyball/stats/2022/fort-valley-state/boxscore/29615
#UCLA vs Fort Valley State
# 25:8
# 25:10
# 25:11

fvs181 <- read.csv2('z18fort_valley_state.csv', sep=";", header = FALSE)[1:34,1]
fvs182 <- read.csv2('z18fort_valley_state.csv', sep=";", header = FALSE)[1:36,2] 
fvs183 <- read.csv2('z18fort_valley_state.csv', sep=";", header = FALSE)[1:37,3]
fvs181
fvs182
fvs183

fvs181_sum<- count_runs_services_sideout(fvs181,1:9) 
fvs181_sum$num_runs
fvs181_sum$A_points
fvs181_sum$B_points
fvs181_sum$n_A
fvs181_sum$n_B
fvs181_sum$A_services
fvs181_sum$B_services

fvs182_sum <- count_runs_services_sideout(fvs182,1:9) 
fvs182_sum$num_runs
fvs182_sum$A_points
fvs182_sum$B_points
fvs182_sum$n_A
fvs182_sum$n_B
fvs182_sum$A_services
fvs182_sum$B_services

fvs183_sum <- count_runs_services_sideout(fvs183,1:9)
fvs183_sum$num_runs
fvs183_sum$A_points
fvs183_sum$B_points
fvs183_sum$n_A
fvs183_sum$n_B
fvs183_sum$A_services
fvs183_sum$B_services

num_runs <- rbind(rbind(fvs181_sum$num_runs,fvs182_sum$num_runs),fvs183_sum$num_runs)
colSums(num_runs)

#Pre prvy set
(fvs181_sum$n_A)/(fvs181_sum$n_A+(fvs181_sum$B_points- fvs181_sum$n_B))
(fvs181_sum$n_B)/(fvs181_sum$n_B+(fvs181_sum$A_points- fvs181_sum$n_A))

P1 <- matrix(c(0.6666667,1-0,1-0.6666667,0), nrow=2, ncol=2)
print(P1)

#Pre druhy set
(fvs182_sum$n_A)/(fvs182_sum$n_A+(fvs182_sum$B_points- fvs182_sum$n_B))
(fvs182_sum$n_B)/(fvs182_sum$n_B+(fvs182_sum$A_points- fvs182_sum$n_A))

P2 <- matrix(c(0.72,1-0.3,1-0.72,0.3), nrow=2, ncol=2)
print(P2)

#Pre treti set
(fvs183_sum$n_A)/(fvs183_sum$n_A+(fvs183_sum$B_points- fvs183_sum$n_B))
(fvs183_sum$n_B)/(fvs183_sum$n_B+(fvs183_sum$A_points- fvs183_sum$n_A))

P3 <- matrix(c(0.5416667,1-0,1-0.5416667,0), nrow=2, ncol=2)
print(P3)
...
#Simulacia prvy set
fvs181_sim_1000 <- sim_volley_games_proc(P1, max_points=25, num_games=1000, A_points=25, B_points=8)
#Simulacia druhy set
fvs182_sim_1000 <- sim_volley_games_proc(P2, max_points=25, num_games=1000, A_points=25, B_points=10)
#Simulacia treti set
fvs183_sim_1000 <- sim_volley_games_proc(P3, max_points=25, num_games=1000, A_points=25, B_points=11)

num_runs_sim <- rbind(rbind(fvs181_sim_1000,fvs182_sim_1000),fvs183_sim_1000)
